...
Expected: every assertion FAILs (script missing), `failed=8`, exit 1.
...
empty() { if [ -z "$2" ]; then echo "ok: $1"; pass=$((pass+1));
  else echo "FAIL: $1 (expected empty, got: $2)"; fail=$((fail+1)); fi; }
hook() { printf '{"cwd":"%s","session_id":"t"}' "$1" | bash "$script"; }

repo="$tmp/repo"; mkdir -p "$repo/sub"; git -C "$repo" init -q
empty "no handoff files: silent" "$(hook "$repo")"

touch "$repo/.handoff-2026-07-13-aaaa.md"
contains "one file: named"        ".handoff-2026-07-13-aaaa.md" "$(hook "$repo")"
contains "instructs resume"       "sdlc:resume"                 "$(hook "$repo")"
contains "found from subdir"      ".handoff-2026-07-13-aaaa.md" "$(hook "$repo/sub")"

touch "$repo/.handoff-2026-07-14-bbbb.md"
out=$(hook "$repo")
contains "many: lists first"  ".handoff-2026-07-13-aaaa.md" "$out"
contains "many: lists second" ".handoff-2026-07-14-bbbb.md" "$out"

plain="$tmp/plain"; mkdir -p "$plain"; touch "$plain/.handoff-2026-07-13-cccc.md"
contains "non-repo cwd works" ".handoff-2026-07-13-cccc.md" "$(hook "$plain")"
...
Expected: all 7 assertions FAIL (script missing), exit 1.
